...
export const TokenChartProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useDataStreamListener(
    ...
    useCallback((get, set, msg) => {
      ...
      // Silently return if chart is not initialized yet - this is expected
      if (!resolution || !baseAsset) {
        return;
      }
      
      // Check if we have initial bar data for this token
      const mostRecentBarKey = baseAsset.id;
      const recentBar = resolutionToMostRecentBar[mostRecentBarKey];
      if (!recentBar) {
        // Chart hasn't loaded initial data yet - this is expected during initialization
        return;
      }
      ...
      if (filteredTxs.length === 0) {
        // No valid transactions to display - silently return
        return;
      }
      ...
      // recentBar was already checked above, so we can proceed
      ...
      if (!nextBar) {
        // No new bar to update - silently return
        return;
      }
      ...
      if (!callback) {
        // Chart subscription not ready yet - silently return
        return;
      }
      callback(nextBar);
    }, [])
