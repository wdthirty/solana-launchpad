  ...
  return {
    ...
    resolveSymbol: async (_, onSymbolResolvedCallback) => {
      ...
      const symbolInfo: ISymbolInfo = {
        ...
        pricescale: 10 ** 16,
        visible_plots_set: 'ohlc', // ohlc = no volume display (replaces deprecated has_no_volume)
    ...
    getBars: async (_, resolution, periodParams, onHistoryCallback, onErrorCallback) => {
      ...
      try {
        ...
        if (rawBars.length === 0) {
          // No trading data available for this token yet - this is expected for new tokens
