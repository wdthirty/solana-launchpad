...
export function patchStreamPool<T extends Pool>(streamedPool: Pool, existingPool: T): T {
  ...
  // Preserve is_migrated and bonding_curve_progress from existing pool
  // These come from our Supabase DB (via Ably or initial fetch), not from Jupiter stream
  // Once a token is migrated, it stays migrated - this is the source of truth
  const existingAny = existingPool as any;
  const preservedFields: Record<string, any> = {};
  if (existingAny.is_migrated !== undefined) {
    preservedFields.is_migrated = existingAny.is_migrated;
  }
  if (existingAny.bonding_curve_progress !== undefined) {
    preservedFields.bonding_curve_progress = existingAny.bonding_curve_progress;
  }
